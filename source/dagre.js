...
dagre.layout = (graph, options) => {
    ...
    // Constructs a new render from the input render, which can be used for layout.
    // This process copies only whitelisted attributes from the input render to the
    // layout render. Thus this function serves as a good place to determine what
    ...
    const runLayout = (g, time) => {
        ...
        // Adds a dummy node to the render and return v.
        ...
        // Assigns a rank to each node in the input render that respects the 'minlen' constraint specified on edges between nodes.
        ...
        const rank = (g) => {
            ...
            //
            // Returns a tree (undirected render) that is constructed using only 'tight' edges.
            ...
            // Initializes ranks for the input render using the longest path algorithm.
            ...
            //
            // Pre-conditions:
            //    1. Input render is a DAG.
            //    2. Input render node labels can be assigned properties.
            //
            // Post-conditions:
            ...
            // The network simplex algorithm assigns ranks to each node in the input render
            // and iteratively improves the ranking to reduce the length of edges.
            //
            // Preconditions:
            //    1. The input render must be a DAG.
            //    2. All nodes in the render must have an object value.
            //    3. All edges in the render must have 'minlen' and 'weight' attributes.
            //
            // Postconditions:
            //    1. All nodes in the render will have an assigned 'rank' attribute that has
            ...
            //       tree edge could be added to produce a more compact render.
            //
            // Much of the algorithms here are derived from Gansner, et al., 'A Technique
            // for Drawing Directed Graphs.' The structure of the file roughly follows the
            // structure of the overall algorithm.
            const networkSimplex = (g) => {
                // Returns a new render with only simple edges. Handles aggregation of data associated with multi-edges.
                const simplify = (g) => {
                    const graph = new dagre.Graph();
                    graph.options = g.options;
                    for (const node of g.nodes.values()) {
                ...
                const initCutValues = (t, g) => {
                    ...
                    for (const v of vs.slice(0, vs.length - 1)) {
                        // Given the tight tree, its render, and a child in the render calculate and
                        // return the cut value for the edge between the child and its parent.
                        const childLabel = t.node(v).label;
                        const parent = childLabel.parent;
                        // The render's view of the tree edge we're inspecting
                        const edge = g.edge(v, parent);
                        // True if the child is on the tail end of the edge in the directed render
                ...
                const enterEdge = (t, g, edge) => {
                    ...
                    // head, so if we don't have this edge in the render we should flip it to
        ...
        // A nesting render creates dummy nodes for the tops and bottoms of subgraphs,
        // adds appropriate edges to ensure that all cluster nodes are placed between
        // these boundries, and ensures that the render is connected.
        // In addition we ensure, through the use of the minlen property, that nodes
        // and subgraph border nodes do not end up on the same rank.
        //
        // Preconditions:
        //    1. Input render is a DAG
        //    2. Nodes in the input render has a minlen attribute
        //
        // Postconditions:
        //   1. Input render is connected.
        //   2. Dummy nodes are added for the tops and bottoms of subgraphs.
        //   3. The minlen attribute for nodes is adjusted to ensure nodes do not
        //      get placed on the same rank as subgraph border nodes.
        //
        // The nesting render idea comes from Sander, 'Layout of Compound Directed Graphs.'
        ...
        const assignRankMinMax = (g) => {
            // Adjusts the ranks for all nodes in the render such that all nodes v have rank(v) >= 0 and at least one node w has rank(w) = 0.
        ...
        // Breaks any long edges in the render into short segments that span 1 layer each.
        // This operation is undoable with the denormalize function.
        //
        // Pre-conditions:
        //   1. The input render is a DAG.
        //   2. Each node in the render has a 'rank' property.
        //
        // Post-condition:
        //   1. All edges in the render have a length of 1.
        //   2. Dummy nodes are added where edges have been split into segments.
        //   3. The render is augmented with a 'dummyChains' attribute which contains
        ...
        // Applies heuristics to minimize edge crossings in the render and sets the best order solution as an order attribute on each node.
        //
        // Pre-conditions:
        //    1. Graph must be DAG
        ...
        const order = (g) => {
            const sortSubgraph = (g, v, cg, biasRight) => {
                // Given a list of entries of the form {v, barycenter, weight} and a constraint render this function will resolve any conflicts between the constraint render and the barycenters for the entries.
                // If the barycenters for an entry would violate a constraint in the constraint render then we coalesce the nodes in the conflict into a new node that respects the contraint and aggregates barycenter and weight information.
                ...
                //    The list `vs` may either be a singleton or it may be an aggregation of nodes ordered such that they do not violate constraints from the constraint render.
            ...
            // ordererd nodes) and a render and returns a weighted crossing count.
            //
            // Pre-conditions:
            //    1. Input render must be simple (not a multigraph), directed, and include
            //       only simple edges.
            //    2. Edges in the input render must have assigned weights.
            //
            // Post-conditions:
            //    1. The render and layering matrix are left unchanged.
            //
            ...
            // Constructs a render that can be used to sort a layer of nodes.
            // The render will contain all base and subgraph nodes from the request layer in their original
            // hierarchy and any edges that are incident on these nodes and are of the type requested by the 'relationship' parameter.
            //
            // Nodes from the requested rank that do not have parents are assigned a root node in the output render,
            // which is set in the root render attribute.
            // This makes it easy to walk the hierarchy of movable nodes during ordering.
            //
            // Pre-conditions:
            //    1. Input render is a DAG
            //    2. Base nodes in the input render have a rank attribute
            //    3. Subgraph nodes in the input render has minRank and maxRank attributes
            //    4. Edges have an assigned weight
            //
            // Post-conditions:
            //    1. Output render has all nodes in the movable rank with preserved hierarchy.
            //    2. Root nodes in the movable layer are made children of the node
            //       indicated by the root attribute of the render.
            //    3. Non-movable nodes incident on movable nodes, selected by the
            //       relationship parameter, are included in the render (without hierarchy).
            //    4. Edges incident on movable nodes, selected by the relationship parameter, are added to the output render.
            //    5. The weights for copied edges are aggregated as need, since the output render is not a multi-render.
        ...
        const position = (g) => {
            ...
            const verticalAlignment = (layering, conflicts, neighborFn) => {
                ...
                const align = {};
                const pos = {};
                // We cache the position here based on the layering because the render and layering may be out of sync.
            ...
            const horizontalCompaction = (g, layering, root, align, reverseSep) => {
                ...
                // Instead of their algorithm we construct a new block render and do two sweeps.
            ...
            // Marks all edges in the render with a type-1 conflict with the 'type1Conflict' property.
    ...
    // Copies final layout information from the layout render back to the input render.
    // This process only copies whitelisted attributes from the layout render to the input render,
...
dagre.Graph = class {
    ...
    setParent(v, parent) {
        if (!this._compound) {
            throw new Error('Cannot set parent in a non-compound render');
        }
        if (parent) {
