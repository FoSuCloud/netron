...
tf.ModelFactory = class {
    ...
    match(context) {
        ...
        if (extension === 'json') {
            for (const type of [ 'json', 'json.gz' ]) {
                ...
                if (obj && obj.modelTopology && (obj.format === 'render-model' || Array.isArray(obj.modelTopology.node))) {
                    return 'tf.' + type;
                }
            }
        }
    ...
    async open(context, target) {
        ...
        const openJson = async (context, type) => {
            try {
                const obj = context.open(type);
                const format = 'TensorFlow.js ' + (obj.format || 'render-model');
                ...
                throw new tf.Error('File text format is not TensorFlow.js render-model (' + error.message + ').');
            }
        };
